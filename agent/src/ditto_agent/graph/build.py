...
def build_graph(
    ...
    # use_verify 기본값 False — gpt-5-mini 실측에서 verify_ambiguities_node가 precision을
    # 0.679→0.500으로 악화시키는 게 확인돼(recall은 소폭만 오름, FP가 거의 2배) 기본
    # 파이프라인에서 뺐다. 노드/LLMClient.verify() 자체는 테스트로 검증된 채 남겨두고,
    # 프롬프트를 더 보수적으로 튜닝한 뒤 `build_graph(use_verify=True)`로 재검증하는 걸
    # 다음 단계로 남김.
    #
    # use_consistency 기본값 False(최종 확정) — golden.json의 고정 few-shot leave-one-out
    # 유출을 다 막고 6회 반복 측정(pooled n=108)한 결과, 진짜 recall/precision 트레이드오프로
    # 확인됨: 끔=recall 0.810/precision 0.761, 켬=recall 0.746/precision 0.825. 오해 방지
    # 도구는 놓치는 것(FN)이 조용히 실패하는 게 과탐지(FP)보다 치명적이라 recall을 우선 —
    # 게다가 켬은 응답을 n배 생성해야 해서 지연시간도 늘어남. precision을 더 우선해야 하는
    # 사용 사례가 있으면 `use_consistency=True`로 언제든 켤 수 있음(옵션은 그대로 남아있음).
    #
    ...
    # 되돌림. 리키지 없는 재검증 방법을 찾기 전까진 이 상태 유지.
