...
## 2026-08-17 (계속) — RAG on/off 첫 동일 모델 비교 → 채택 보류

사용자가 "비어있는 모델 중 하나로 RAG 해보자"고 요청 — o3-mini는 이미 프로덕션
모델로 많이 써서 제외, 오늘 전혀 안 쓴 **o3(풀사이즈)** 로 진행(RPM 3/min이라
gpt-4.1처럼 pace=21초 필요).

같은 20케이스(T01~F05)로 RAG 끄고/켜고 각각 완주:

| 조건 | recall | precision | FP |
|---|---|---|---|
| RAG 끔 | 1.000 | 0.500 | 10 |
| RAG 켬 | 1.000 | 0.455 | 12 |

**처음으로 나온 깨끗한 동일 모델·동일 케이스 비교 — RAG가 도움 안 됨, FP만 늘어남
(precision 0.500→0.455)**. `use_rag=False`(기본 꺼짐)를 유지하는 게 맞았던
결정으로 확인됨 — verify-loop과 같은 패턴(설계 의도와 반대 결과 → 기본값 안 켬).
o3 자체가 과탐지 경향이 강한 모델(RAG 없이도 precision 0.500)이라 이 결과가
다른 모델에도 그대로 적용되는지는 조심스럽지만, 오늘 확보한 유일한 직접 비교
데이터로는 RAG 채택 근거 없음. 상세: `docs/survey-results-analysis.md` 17-1절.
